...
<div class="input-group input-xlarge">
    <span class="input-group-addon"><i id="${id}icon" class="${value}"></i></span>
    <input type="text" id="${id}" name="${name}" value="${value}" readonly="true" class="form-control"/>
	<span class="input-group-btn">
		<button id="${id}search" class="btn btn-sm btn-primary" type="button">
            <i class="icon-search"></i>
        </button>
		<button id="${id}clear" class="btn btn-sm btn-warning" type="button">
...
<script type="text/javascript">
    ...
    $("#icons li").click(function () {
        ...
        if ($(this).text() != 'icon-') {
            $("#${id}value").val($(this).text());
        }
    });
    $("#icons li").each(function () {
        if ("icon-" + $(this).text() == "${value}") {
            $(this).click();
        }
    });
    $("#icons li").dblclick(function () {
        document.getElementById('${id}dialog-select-button').click();
    });
    function ${id}doGetSelectResult() {
        var result = "";
        if ($('#${id}value').val() != "") {
            result = 'icon-' + $('#${id}value').val();
        }
        return result;
    }
    function ${id}popupMenuIcon() {
        $("#${id}popup").removeClass('hide').dialog({
            modal: true,
            title: "<div class='widget-header widget-header-small'><h4 class='smaller'>Select Icon</h4></div>",
            title_html: true,
            width: '650',
            height: '450',
            buttons: [
                {
                    id: '${id}dialog-select-button',
                    html: "<i class='icon-ok'></i>&nbsp; Select",
                    "class": "btn btn-primary btn-xs",
                    click: function () {
                        var result = ${id}doGetSelectResult();
                        $("#${id}").val(result);
                        $('#${id}icon').attr('class', result);
                        $(this).dialog("close");
                    }
                },
                {
                    html: "<i class='icon-off'></i>&nbsp; Close",
